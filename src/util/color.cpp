...
Color Color::fromHsv(const DataType& h, const DataType& s, const DataType& v) {
    DataType C = s * v;
    auto X = static_cast<DataType>(C * (1.0 - std::abs(std::fmod(h / 60.0, 2.0) - 1.0)));
    DataType m = v - C;
    DataType Rs, Gs, Bs;
...
void Color::toHsv(DataType* hue, DataType* sat, DataType* val) const {
    ...
    if (fDelta > 0) {
        if (fCMax == red()) {
            ...
            *hue = static_cast<DataType>(60 * (((blue() - red()) / fDelta) + 2));
        } else if (fCMax == blue()) {
            *hue = static_cast<DataType>(60 * (((red() - green()) / fDelta) + 4));
        }
...
void Color::toHsl(DataType* hue, DataType* sat, DataType* lightness) const {
    ...
    *lightness = static_cast<DataType>(0.5 * (fCMin + fCMax));
    ...
    if (fCMin == fCMax) {
        ...
        *sat = static_cast<DataType>((fCMax - fCMin) / (2.0 - fCMax - fCMin));
    }
    ...
    for (unsigned int i = 0; i < count; i++) {
        ...
        switch (format) {
            ...
            case PixelFormat::COUNT:
                break;
            case PixelFormat::U8:
                reinterpret_cast<uint8_t*>(out)[i] = static_cast<uint8_t>(f * 255.0);
                break;
            case PixelFormat::U16:
                reinterpret_cast<uint16_t*>(out)[i] = static_cast<uint16_t>(f * 65535.0);
                break;
    ...
    for (unsigned int i = 0; i < count; i++) {
        ...
        switch (format) {
            ...
            case PixelFormat::COUNT:
                break;
            case PixelFormat::U8:
                f = static_cast<DataType>(DataType(reinterpret_cast<const uint8_t*>(in)[i]) / 255.0);
                break;
            case PixelFormat::U16:
                f = static_cast<DataType>(DataType(reinterpret_cast<const uint16_t*>(in)[i]) / 65535.0);
                break;
            case PixelFormat::F16:
...
Color::DataType Color::GetRoughLuminance() const {
    return static_cast<DataType>((2 * red() + blue() + 3 * green()) / 6.0);
}
