...
TimeRange::TimeRange(const rational& in, const rational& out) : in_(in), out_(out) {
    normalize();
}
...
bool TimeRange::Contains(const TimeRange& a, bool in_inclusive, bool out_inclusive) const {
    bool contains_in = (in_inclusive) ? (a.in() >= in()) : (a.in() > in());

    bool contains_out = (out_inclusive) ? (a.out() <= out()) : (a.out() < out());
...
TimeRange TimeRange::Combine(const TimeRange& a, const TimeRange& b) {
    return {std::min(a.in(), b.in()), std::max(a.out(), b.out())};
}
...
TimeRange TimeRange::Intersect(const TimeRange& a, const TimeRange& b) {
    return {std::max(a.in(), b.in()), std::min(a.out(), b.out())};
}
...
std::list<TimeRange> TimeRange::Split(const int& chunk_size) const {
    ...
    int start_time = static_cast<int>(std::floor(this->in().toDouble() / static_cast<double>(chunk_size)) * chunk_size);
    int end_time = static_cast<int>(std::ceil(this->out().toDouble() / static_cast<double>(chunk_size)) * chunk_size);
...
TimeRangeListFrameIterator::TimeRangeListFrameIterator(TimeRangeList list, const rational& timebase)
    ...
    if (!list_.isEmpty() && timebase_.isNull()) {
        Log::Warning()
            << "TimeRangeListFrameIterator created with null timebase but non-empty list, marking as exhausted";
        list_ = TimeRangeList();
    }
...
int TimeRangeListFrameIterator::size() {
    if (size_ == -1) {
        ...
        for (const TimeRange& range : list_) {
            ...
            size_ += static_cast<int>(1 + (end_ts - start_ts));
        }
    }
